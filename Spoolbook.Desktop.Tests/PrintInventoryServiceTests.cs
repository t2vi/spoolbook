...
{
    private static async Task<(int bambuSpoolId, int prusaSpoolId, int profileId)> SeedAsync(SpoolbookDbContext db)
    {
        ...
        return (bambuSpool.Spool!.Id, prusaSpool.Spool!.Id, profile.Profile!.Id);
    }
    ...
    public async Task List_ReturnsAllPrintsByDefault()
    {
        ...
        var (bambuSpoolId, prusaSpoolId, profileId) = await SeedAsync(db);
        var printService = new PrintService(db, new FakeWeatherService());
        await printService.CreateAsync(profileId, bambuSpoolId, new PrintInput { Printer = "P2S", StartedAt = new DateTime(2026, 1, 1), EndedAt = new DateTime(2026, 1, 1, 2, 0, 0), Status = PrintStatus.Success });
        await printService.CreateAsync(profileId, prusaSpoolId, new PrintInput { Printer = "P2S", StartedAt = new DateTime(2026, 1, 2), EndedAt = new DateTime(2026, 1, 2, 2, 0, 0), Status = PrintStatus.Failed });
    ...
    {
        using var db = TestDbFactory.Create();
        var (bambuSpoolId, _, profileId) = await SeedAsync(db);
        var printService = new PrintService(db, new FakeWeatherService());
        await printService.CreateAsync(profileId, bambuSpoolId, new PrintInput { Printer = "P2S", StartedAt = new DateTime(2026, 1, 1), EndedAt = new DateTime(2026, 1, 1, 2, 0, 0), Status = PrintStatus.Success });
        await printService.CreateAsync(profileId, bambuSpoolId, new PrintInput { Printer = "P2S", StartedAt = new DateTime(2026, 1, 5), EndedAt = new DateTime(2026, 1, 5, 2, 0, 0), Status = PrintStatus.Success });
        var service = new PrintInventoryService(db);
